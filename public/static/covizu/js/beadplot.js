...
var xValueB = function (d) {
  ...
  xMapB = function (d) {
    return xScaleB(xValueB(d));
  };

var yValueB = function (d) {
    return d.y;
  },
  yValue1B = function (d) {
    return d.y1;
  ...
  yMapB = function (d) {
    return yScaleB(yValueB(d));
  };

var visB = d3
  .select('div#svg-cluster')
  .append('svg')
...
async function beadplot(cid) {
  ...
  slider.slider('value', 2.0).slider('option', 'max', max_dist);
  move_arrow();
  ...
  function redraw() {
    ...
    visB
      ...
      .on('mouseout', function (d) {
        ...
        if (!parent_variant.empty()) {
          ...
          if (
            this.classList.contains('selectionL') ||
            parent_variant.node().classList.contains('selectionLH')
          ) {
            parent_variant.attr('stroke-width', 5);
          } else {
        ...
        if (!child_variant.empty()) {
          ...
          if (
            this.classList.contains('selectionL') ||
            child_variant.node().classList.contains('selectionLH')
          ) {
            child_variant.attr('stroke-width', 5);
          } else {
      ...
      .on('mouseover', function (d) {
        if (d.label !== null) {
          ...
          if (!d.unsampled) {
            ...
            tooltipText += `<b>${i18n_text.hedge_coldates}:</b><br>${formatDate(
              d.x1,
            )} / ${formatDate(d.x2)}`;
          }
    ...
    var dateDiff = d3.timeDay.count(
      d3.min(variants, function (d) {
        return d.x1;
      }),
      d3.max(variants, function (d) {
        return d.x2;
      }),
    );
...
function gen_details_table(obj) {
  ...
  t_rows
    ...
    .on('mouseover', function (x) {
      ...
      if (x.startsWith('EPI_ISL')) {
        gisaid.getAcknowledgementData(x, function (gd) {
          cTooltip.transition().duration(50).style('opacity', 0.9);
    ...
    .on('mouseout', function () {
      cTooltip.transition().duration(50).style('opacity', 0);
    })
    .text(function (x) {
      return x;
    })
...
function gentable(obj) {
  ...
  let resolved = {};
  for (let co in obj.c2r) {
    // country
    let maxcount = 0,
      maxregion = null;
    for (let re in obj.c2r[co]) {
      // region
  ...
  var rows = country_tbody.selectAll('tr').data(my_countries);
  rows.exit().remove();
  rows
    .enter()
    .append('tr')
...
function draw_region_distribution(my_regions) {
  ...
  chart.append('g').call(d3.axisLeft(yScale).ticks(3));

  // Create the chart
  const regionBars = chart.selectAll().data(counts).enter().append('g');
...
function move_arrow() {
  ...
  } else $('#right-arrow').css('margin-left', 0);
}

...
$('#splash-button')
  .parents('.ui-dialog-buttonpane')
  .append(
    '<div style="color: #4F2683" id=\'splash-extra\'>' +
      i18n_text.loading_json +
...
var clusters, beaddata, tips, accn_to_cid, cindex, lineage_to_cid;
var map_cidx_to_id = [],
  id_to_cidx = [];
...
$.ajax({
  ...
  $('#next_button').click(function () {
    ...
    if (
      curr_bead == 0 &&
      parseInt(d3.selectAll('rect.clicked').nodes()[0].id.substring(3)) >
      ...
      if (
        accn_to_cid[bead_id_to_accession[curr_bead]] !==
      ...
      } else select_working_bead(bead_id_to_accession, curr_bead + 1);
  ...
  $(document).on('mousedown', function (e) {
    if (e.target.matches('div.larrow')) $('.larrow').addClass('selected');
    else if (e.target.matches('div.rarrow')) $('.rarrow').addClass('selected');
    else {
  ...
  $(document).on('keyup', function (e) {
    if (
      e.target.matches('div.ui-slider-handle') ||
      $('.larrow').hasClass('selected') ||
      $('.rarrow').hasClass('selected')
    ) {
      if (e.keyCode == 37) $('#left-arrow').removeClass('clicked');
      else if (e.keyCode == 39) $('#right-arrow').removeClass('clicked');
    ...
    if (e.keyCode == 37 || e.keyCode == 39) {
      ...
      if (selected_bead.length == 0) {
        ...
        if (e.keyCode == 37) {
          if (bead_id - 1 >= 0) {
            var prev_node = d3
              .selectAll('circle[idx="' + (bead_id - 1).toString() + '"]')
              .nodes()[0];

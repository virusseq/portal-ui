...
async function wrap_search() {
  ...
  if (isAccn(query)) await accession_search(query);
  else if (isLineage(query)) await lineage_search(query, start_date, end_date);
  else await main_search(beaddata, query, start_date, end_date);
}

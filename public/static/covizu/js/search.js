...
function wrap_search() {
  ...
  } else start_date = utcDate(start_date_text);

  if (end_date_text === '') {
    end_date = new Date();
    $('#end-date').val(formatDate(end_date));
  } else end_date = utcDate(end_date_text);
  ...
  if (isAccn(query)) accession_search(query);
  else if (isLineage(query)) lineage_search(query, start_date, end_date);
  else main_search(beaddata, query, start_date, end_date);
}
  ...
  search_hits = flat_data.filter(function (bead) {
    temp =
      (bead.accessions.some((accession) =>
        accession.toLowerCase().includes(text_query.toLowerCase()),
      ) ||
...
function select_next_prev_bead(bead_id_to_accession, curr_bead) {
  ...
  var next_cluster = d3.selectAll(
    'rect[cidx="cidx-' + accn_to_cid[bead_id_to_accession[curr_bead]] + '"]',
  );
  d3.selectAll('rect.clickedH').remove();
...
function select_bead_by_accession(accn, reset_clusters_tree = true) {
  ...
  if (cid !== undefined) {
    if (reset_clusters_tree === true) {
      d3.selectAll('#svg-timetree > svg > g > rect:not(.clickedH)').attr(
        'class',
        'not_SelectedCluster',
      );
    }
    ...
    var rect = d3
      .selectAll('#svg-timetree > svg > g > rect:not(.clickedH)')
      .filter(function (d) {
        return d.cluster_idx === cid;
      })
      .attr('class', 'clicked');
...
function closest_match(non_hit_cluster_index, hit_ids) {
  ...
  while (i < j) {
    ...
    if (hit_ids[mid] == target_index) return hit_ids[mid];
...
function getClosest(key1, key2, target) {
  if (target - key1 >= key2 - target) return key2;
  else return key1;
}
